...
MLHeuristic(std::string, std::string policy_net_path) {
    //load graph
    Status rt = load_graph("./Asimov_value_net.meta", &tf_sess);
    if (!rt.ok()) {
        LOG(ERROR) << rt;
    }

    rt = load_graph("./Asimov_policy_net.meta", &tf_sess);
    if (!rt.ok()) {
        LOG(ERROR) << rt;
    }
}
...
virtual float evaluate(BitBoard &b, Side s) {
    Tensor bb(DL_FLOAT, tensorflow::TensorShape({1,192}));
 ...
 */
virtual void order_moves(BitBoard &b, Side s, int n, Move *rmvs, std::pair<float, int> *mvs) {
    Tensor bb(DL_FLOAT, tensorflow::TensorShape({1,8,8,4}));
